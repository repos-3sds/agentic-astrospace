...
| Codex | Mobile Ask richer structured rendering | `codex-ask-structured-rendering` | In progress — improve structured answer presentation, section hierarchy, and next-step/action rendering without changing backend contracts | `ui/src/app/features/mobile/ask/*`, `ui/src/app/core/models.ts` if needed | 2026-08-09 |
| Gemini | Children domain — **PR open** (PR #9). Implemented `_CHILDREN_ADDENDUM` with explicit guardrails against predicting progeny count/gender, updated `AGENT_REGISTRY` and tests. Handled via isolated worktree, PR tagged for review. Next up: Health domain (waiting on Ask-hardening safety-regex fixes). | `gemini/children-domain-spec` | In progress (PR #9 review) | `astrospace/agents/registry.py`, `tests/*` | 2026-08-09 |
