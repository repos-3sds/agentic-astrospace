...
|---|---|---|---|---|---|
| Claude | Ask backend Item 2: fix the 44 safety.py regex gaps Qwen's audit found (PR #7) | `claude/safety-paraphrase-fix` | In progress — redesigning `prohibited_verdict()`/`dosha_overclaim_kind()` with contraction normalization + generalized pattern clusters, not phrase-by-phrase patches; removing the `xfail` markers as each case gets a real fix | `astrospace/agents/safety.py`, `tests/test_verifier.py` | 2026-08-09 |
