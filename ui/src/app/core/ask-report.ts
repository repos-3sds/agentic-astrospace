...
export function askReportText(input: AskReportInput): string {
  ...
  const lines: string[] = [
    ...
    ...(input.createdAt ? [`Generated: ${new Date(input.createdAt).toLocaleString()}`] : []),
    '',
    'ACKNOWLEDGMENT',
  ...
  if (reading.guidance.follow_up_questions.length) {
    lines.push('', 'FOLLOW-UP QUESTIONS');
    reading.guidance.follow_up_questions.forEach(question => lines.push(`• ${clean(question)}`));
  }
  ...
  lines.push('', 'Siddha offers reflective Vedic astrology guidance. It does not replace medical, legal, financial, or other qualified professional advice.');
...
function reportBlocks(input: AskReportInput): ReportBlock[] {
  const reading = input.reading;
  if (!reading) return [
    { kind: 'title', text: 'Siddha' },
    { kind: 'subtitle', text: 'Reading' },
    { kind: 'rule' },
  ...
  const blocks: ReportBlock[] = [
    { kind: 'title', text: 'Siddha' },
    { kind: 'subtitle', text: 'Structured Vedic Astrology Reading' },
    { kind: 'rule' },
    ...
    { kind: 'heading', text: 'Technical Basis' },
  ];
  ...
  if (reading.guidance.follow_up_questions.length) {
    blocks.push({ kind: 'heading', text: 'Follow-up Questions' });
    reading.guidance.follow_up_questions.forEach(question => blocks.push({ kind: 'bullet', text: clean(question) }));
  }
  const provenance = [
