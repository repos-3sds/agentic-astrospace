alter table public.knowledge_chunks
  add column if not exists retrieval_scope text not null default 'core',
  add column if not exists retrieval_exclusion_reason text,
  add column if not exists scope_confidence double precision not null default 1.0,
  add column if not exists scope_classifier text not null default 'default-core-v1';

alter table public.knowledge_chunks
  drop constraint if exists knowledge_chunks_retrieval_scope_check;

alter table public.knowledge_chunks
  add constraint knowledge_chunks_retrieval_scope_check
...
create index if not exists knowledge_chunks_retrieval_scope_idx
  on public.knowledge_chunks (retrieval_scope, quality_status);
